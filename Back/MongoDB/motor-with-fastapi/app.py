    ...
    class Config:
        ...
        schema_extra = {
            "example": {
                ...
                "course": "Experiments, Science, \
                            and Fashion in Nanophotonics",
                "gpa": "3.0",
            }
        }
...
async def show_student(id: str):
    if (student := await db["students"].find_one(
            {"_id": id})) is not None:
        return student
...
async def update_student(id: str,
    ...
    student = {k: v for k, v in student.dict().items()
               if v is not None}
    ...
    if len(student) >= 1:
        ...
        if update_result.modified_count == 1:
            if (
                updated_student := await db["students"]
                    .find_one({"_id": id})
            ) is not None:
                return updated_student

    if (existing_student := await db["students"].find_one(
            {"_id": id})) is not None:
...
async def delete_student(id: str):
    delete_result = await db["students"].delete_one(
        {"_id": id})
